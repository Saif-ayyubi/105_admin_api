...
public class ReportServiceImpl implements ReportService{
    ...
    public byte[] generateReport(ReportFilterDto filterDto) throws Exception{
        ...
        if(filterDto.getOrderStatus() != null & !"".equals(filterDto.getOrderStatus())){//check 'OrderStatus' is coming or not
            entity.setOrderStatus(filterDto.getOrderStatus());//if 'OrderStatus' is coming then we are setting here
        }
        List<OrderEntity> ordersList = orderRepo.findAll(Example.of(entity));//retrieving all orders based on 'OrderStatus'
        // Example.of(entity)==> generate Dynamic Query or filter records based on entity

        HSSFWorkbook workbook = new HSSFWorkbook();//creating workbook first//or create excel sheet

        HSSFSheet sheet = workbook.createSheet();//creating sheet in workbook
        HSSFRow headerRow = sheet.createRow(0);//creating 1st row(Header Row)
        headerRow.createCell(0).setCellValue("S.No");//what columns you want to set in Header Row//creating cell n setting cell value
        ...
        int dataRowIndex = 1;//for data row index starts from 1
        int sno = 1;

        for (OrderEntity order : ordersList) {//for each order i'll create one row
        ...
        return bos.toByteArray();//we are returning byte array
    }
}
